...
async function sendInvitation(contact, invitation) {
  ...
  try {
    ...
    // Onthoud per contact wanneer de laatste uitnodiging verstuurd is
    try { await contacts.update(contact.id, { lastInvitedAt: new Date().toISOString() }); }
    catch (e) { console.error('Kon lastInvitedAt niet updaten:', e); }
    return { sent: true };
  } catch (e) {
...
async function processReminders() {
  ...
  if (!s.reminderEnabled) return { sent: 0, skipped: 0, reason: 'disabled' };
  ...
  if (sent > 0) {
    try {
      await audit.log({
        ...
        details: { sent, failed, candidates: candidates.length },
      });
...
async function sendDetractorAlert(entry, contact) {
  ...
  try {
    ...
    try {
      await audit.log({
        actor: 'system',
        action: 'detractor.alert',
        targetType: 'response',
        targetId: entry.id,
        targetLabel: `${name} (${a.q1_overall}/10)`,
        details: { score: a.q1_overall, recipients: DETRACTOR_RECIPIENTS },
      });
    } catch (e) { console.error('Audit error:', e); }
    return { sent: true };
  } catch (e) {
...
async function startRound({ triggeredBy = 'manual', contactIds = null, sendEmail = true } = {}) {
  ...
  if (triggeredBy === 'auto') {
    ...
    try {
      await audit.log({
        actor: 'system',
        action: 'round.start.auto',
        targetType: 'round',
        targetId: round.id,
        targetLabel: `Auto-ronde ${new Date(round.createdAt).toLocaleString('nl-NL')}`,
        details: { total: invitations.length, sent, failed },
      });
    } catch (e) { console.error('Audit error:', e); }
  }
...
// Bepaalt welke contacten "due" zijn op basis van eigen interval-override of de default.
async function dueContacts() {
  ...
  // Map: contactId → meest recente sentAt (voor migratie van bestaande contacten zonder lastInvitedAt)
  const latestSentByContact = {};
  for (const inv of allInvitations) {
    if (!inv.sentAt || !inv.contactId) continue;
    const t = inv.sentAt;
    if (!latestSentByContact[inv.contactId] || latestSentByContact[inv.contactId] < t) {
      latestSentByContact[inv.contactId] = t;
    }
  }

  return activeContacts.filter(c => {
    const interval = (c.intervalDaysOverride && c.intervalDaysOverride > 0)
      ? c.intervalDaysOverride
      : defaultDays;
