...
async function listRounds(type = null) {
  const all = await storage.read('rounds', []);
  if (!type) return all;
  return all.filter(r => (r.type || 'klant') === type);
}
...
async function createRound({ triggeredBy = 'manual', contactIds = [], type = 'klant' } = {}) {
  const rounds = await storage.read('rounds', []);
...
async function markReminderSent(invitationId) {
  ...
  const inv = all.find(i => i.id === invitationId);
  if (!inv) return null;
  // Backward-compat: oudere invitaties hebben alleen reminderSentAt en geen reminderCount.
  const prevCount = (typeof inv.reminderCount === 'number')
    ? inv.reminderCount
    : (inv.reminderSentAt ? 1 : 0);
  inv.reminderCount = prevCount + 1;
