...
async function listActive() {
  ...
  return all.filter(c => c.active !== false && !c.optOut);
}
...
async function create({ name, company, email, accountManager }) {
  ...
  const contact = {
    ...
    accountManager: String(accountManager || '').trim().slice(0, 120),
    active: true,
    optOut: false,
...
async function update(cid, patch) {
  ...
  if (i === -1) return null;
  const allowed = ['name', 'company', 'email', 'active', 'accountManager', 'optOut'];
